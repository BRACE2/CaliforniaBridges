...
def okid(dati, dato, **config):
    ...
    uu,wr,v = svd(U,0);  # svd: Singular Value Decomposition
                        #      s is a diagonal matrix with the singular values
    #wr = s #np.diag(s)     # extract singular values from the diagonal matrix
    pg = (r+m)*p+r
   ...
   #markovParamError = sum(Jpre)/m;  # RMSE between actual output & y on left 
                                     # hand side in Eq. 3.76. It should be 
                                     # quite small (e.g., 10^-3) for accurately 
                                     # computed Markow parameters
